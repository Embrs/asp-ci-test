...
public static class AuthApis {
	public static void InitAuthApis(this WebApplication app) {
		...
		// Register
		app.MapPost("/auth/register", async (AppDbContext db,RegisterParams apiParams) => {
		...
		// Login
		app.MapPost("/auth/login", async (AppDbContext db, LoginParams apiParams, JwtService jwtService) => {
